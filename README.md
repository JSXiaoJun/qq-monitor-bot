...
利润等于“本站消耗 - 上游总和”，所有金额保留两位小数。机器人会在每次 `查余额` 时静默刷新一次上游今日消耗。查询利润时，实时查询失败的站点会使用当天最近一次成功结果，并动态标注 `35分钟前`、`2小时前` 等相对时间；当天从未成功的站点显示“查询失败（0）”并按 `0` 计入上游总和，不会中断其他站点的汇总。上游站点需在 `upstream-ratio-watch` 中启用登录检测并配置有效凭据。
